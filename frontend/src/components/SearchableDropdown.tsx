    ...
    <div className="lang-select" ref={ref}>
      ...
      >
        <span className="lang-select-value">
          <Icon name={value === 'auto' ? 'search' : 'globe'} size={16} />
          {' '}
          {displayName}
        </span>
        <span className={`lang-select-arrow ${isOpen ? 'up' : ''}`}>
      ...
      {isOpen && (
        <div className={`lang-select-menu ${darkMode ? 'dark' : 'light'}`}>
          <div className="lang-select-search">
            <Icon name="search" size={16} className="lang-select-search-icon" />
            <input
              ref={inputRef}
              type="text"
          ...
          <div className="lang-select-options" ref={optionsRef}>
            {filtered.map(({ code, name }, idx) => (
              ...
              >
                <span className="lang-select-option-name">
                  {code === 'auto' ? (
                    <><Icon name="search" size={14} /> Auto Detect</>
                  ) : (
                    name
                  )}
                </span>
                ...
                {value === code && (
                  <span className="lang-select-check">
